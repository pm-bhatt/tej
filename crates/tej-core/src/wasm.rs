...
async fn measure_latency_js() -> Result<(WasmLatencyResult, Option<String>), JsValue> {
    ...
    let min = *latencies.iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap_or(&0.0);
    let max = *latencies.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap_or(&0.0);
